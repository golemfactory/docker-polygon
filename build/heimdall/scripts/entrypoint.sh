...
then
    ...
    # BOR
    sed -i "s#^bor_rpc_url.*#bor_rpc_url = \"http://bor:8545\"#" config.toml
    # ETH1
    sed -i "s#^eth_rpc_url.*#eth_rpc_url = \"${ETH1_RPC_URL}\"#" config.toml
    # RABBITMQ
    sed -i "s#^amqp_url.*#amqp_url = \"amqp://guest:guest@rabbitmq:5672\"#" config.toml
fi
